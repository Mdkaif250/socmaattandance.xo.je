...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Basic Validation
    if (empty($prn) || empty($name) || empty($email) || empty($password)) {
        ...
        $stmt = $conn->prepare("SELECT prn FROM users WHERE prn = ? OR email = ?");
        $stmt->bind_param("ss", $prn, $email);
        $stmt->execute();
        $stmt->store_result();
        ...
        if ($stmt->num_rows > 0) {
            ...
            // All good, create the teacher
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);
            $role = 'teacher';
            
            $stmt_insert = $conn->prepare("INSERT INTO users (prn, name, email, password, role) VALUES (?, ?, ?, ?, ?)");
            $stmt_insert->bind_param("sssss", $prn, $name, $email, $hashed_password, $role);

            if ($stmt_insert->execute()) {
                $success = true;
                $message = "Teacher account for '" . htmlspecialchars($name) . "' created successfully!";
            } else {
                $message = "An error occurred. Could not create account.";
            }
            $stmt_insert->close();
        }
        $stmt->close();
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn { width: 100%; padding: 15px; border: none; border-radius: 8px; background-color: #3498db; color: #fff; font-size: 16px; cursor: pointer; }
...
<body>
    <?php
    // This small PHP block dynamically sets the 'active' class on the sidebar link
    ...
    <div class="sidebar">
        ...
        <ul class="nav-links">
            ...
            <li><a href="add-teacher.php" class="active"><i class="fas fa-user-tie"></i> Add Teacher</a></li>
    ...
    <div class="main-content">
        <div class="header"><h1>Add New Teacher</h1></div>
        ...
        <div class="card">
            ...
            <form action="add_teacher.php" method="post" novalidate>
                <div class="form-group">
                    <label for="prn">Teacher PRN / ID</label>
                    <input type="text" id="prn" name="prn" placeholder="e.g., T001" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="password">Initial Password (min. 6 characters)</label>
